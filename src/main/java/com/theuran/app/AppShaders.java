...
public class AppShaders {
    ...
    public AppShaders(ShaderPipeline pipeline) {
    ...
    public void reload() {
        ...
        for (ShaderStage stage : this.pipeline.stages) {
            List<Texture> output = new ArrayList<>();

            for (String outputTexture : stage.output) {
                output.add(this.textures.get(outputTexture).texture);
            }

            Shader stageShader = new Shader(stage.shader, VBOAttributes.VERTEX_2D);
            ...
            stageShader.onInitialize(shader -> {
                ...
                for (String bufferName : stage.input) {
